...
*   **解决方案**：有两种方案进行公式排版：
    1.  **CodeCogs 渲染方案**：将独立的公式块替换为 **CodeCogs 渲染的高清 SVG 矢量图片**，用 HTML `<p align="center">` 标签包裹 `<img>`。LaTeX 源码开头必须添加 `\bg_white`，并将公式内容**完全 URL 编码**（尤其是括号 `(` 编码为 `%28`，`)` 编码为 `%29`，否则 GitHub 的渲染机制会解析失败返回 503）。
    2.  **本地图片渲染方案（推荐用于复杂公式）**：由于 CodeCogs 服务器极不稳定且常有 502/503 错误，对于新公式，应通过本地 Python 脚本（利用 matplotlib / sympy）渲染出高分辨率的 PNG 图片，存放在对应的 `images/` 目录下（必须设置白色底面以适应暗色模式），并在 README 中以相对路径引用（例如 `images/eq1_ssi.png`）。
