...
$$
\mathcal{L}_{\text{ssi}} = \frac{1}{N} \sum_{i} \left( \hat{d}_i^{\text{pred}} - \hat{d}_i^{\text{GT}} \right)^2, \quad \hat{d} = \frac{d - t(d)}{s(d)}
$$
...
$$
\mathcal{L}_{\text{gm}} = \frac{1}{K} \sum_{k=1}^{K} \left( \left| \nabla_x d^{\text{pred}}_k - \nabla_x d^{\text{GT}}_k \right| + \left| \nabla_y d^{\text{pred}}_k - \nabla_y d^{\text{GT}}_k \right| \right)
$$
...
$$
\mathcal{L}_{\text{tc}} = \sum_t \mathbb{E}_{i \in \mathcal{M}_t} \left[ \left( \tilde{d}_t^i - d_{t+1}^i \right)^2 \right]
$$
...
$$
f_s^{\text{enriched}} = f_s^{\text{RGB}} + \alpha_s \cdot \phi_s(\text{LiDAR})
$$
