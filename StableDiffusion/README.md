...
*   <strong>核心机制</strong>:
    ...
    3.  <strong>公式符号极简解释</strong>:
        ...
        2.  `p_t`: 时间维度切片的大小（Patch length in frames），即每次连续截取的帧数。
        3.  `p_s`: 空间分块的尺寸大小。
        4.  `N`: 最终拉伸成一维序列后的 Token 数量。例如，16 帧、大小为 32x32 的特征图，若设 `p_t=2, p_s=4`，则会被切分成 `(16/2) * (32/4) * (32/4) = 8 * 8 * 8 = 512` 个 3D patches，每个 patch 映射为一维向量，附带 3D 时空位置编码送入模型中。

### 6.1 Video DiT（时空物理世界模型）整体工作流
为完整展示时空降采样、分块、自注意力与文本交叉注意力的运作，参考以下工作流图：
<p align="center"><img src="images/video_dit_architecture.png" width="85%" alt="Video DiT Pipeline" /></p>

*   <strong>工作流模块拆解解释</strong>:
    1.  <strong>3D VAE Encoder (左上)</strong>: 接收高维视频数据 `[B x F x C x H x W]`，在空间下采样 8x 并在时间下采样 2x，去除画面之间的冗余，生成 3D Latents。
    2.  <strong>3D Patchification (左中)</strong>: 用 3D 卷积核将 3D 隐视频块投影并压扁成一维 Tokens 序列。
    3.  <strong>3D Positional Embeddings (中下)</strong>: 在 sequence 中注入 3D 时空位置编码，让模型感知图像内的上下左右方位以及帧之间的先来后到顺序。
    4.  <strong>Spatiotemporal DiT Blocks (右中)</strong>: tokens 经过堆叠的 Transformer 处理。每个 Block 内部除了全局时空双向注意力（Self-Attention）以建立物理演变规律外，还引入了 <strong>Cross-Attention 机制</strong>。
    5.  <strong>Text prompt 注入 (右下)</strong>: 外部的文本提示词经过大语言模型提取出 text condition，在 Cross-Attention 阶段作为 Key/Value 注入，决定时空画面的渲染细节。
    6.  <strong>3D VAE Decoder (左下)</strong>: tokens 经 Unpatchify 还原成隐视频，最后通过 Decoder 重构为像素级视频画面输出。
...
### 8.3 RDT-1B 异构模态 Token 拼接与大模型推理
为更具体地观察各路模态信号（本体、语言、多视角图像、加噪动作）是如何汇聚拼接的，请参见下方多模态数据流动架构图：
<p align="center"><img src="images/rdt_architecture.png" width="85%" alt="RDT-1B Multimodal Flow" /></p>

*   <strong>工作流模块拆解解释</strong>:
    1.  <strong>Multimodal Inputs (左侧列)</strong>: 本体状态（state_dim维）、T5 编码后的控制文字口令（4096维）、SigLIP 提取的各机位画面（1152维）和带噪声动作（128维）同时输入。
    2.  <strong>Modality Projectors (中间列)</strong>: 针对不同模态的高维度数据，RDT 在每个通道处放置了一个轻量级的线性 Projection 层（即 Adapter，包含 State, Text, Vision, Action 四个独立的投影投影头），将其统一向隐维度 `D`（即 Backbone 的 Embedding width）对齐。
    3.  <strong>Concatenate Tokens (灰色长矩形)</strong>: 拼接公式如下图所示：
        <p align="center"><img src="images/eq10_rdt_concat.png" width="75%" alt="RDT Token Concatenation" /></p>
        拼接结果形成一个 `[State, Language, Vision, Action]` 混合大序列 `S`（形状为 `[B x N_total x D]`），其中包含：1个 state token，`L_lang` 个文字 tokens，`L_img` 个图像 tokens 和 `T_p` 个动作 tokens。
    4.  <strong>RDT Backbone (紫色，右中)</strong>: 由大量 DiT block 堆叠而成的 Transformer。拼接后的长序列在双向自注意力机制中，进行完全无偏阻的多模态交互。同时，当前的扩散步数 `t` 通过 AdaLN 归一化层动态回归，调制 Backbone 中每个 block 的内部特征激活。
    5.  <strong>Predict Action Noise (右下)</strong>: 从 Backbone 输出的长序列中，仅切出最后 `T_p` 个属于 Action 位置的 tokens，并输入 MLP 投影预测出动作去噪噪声。
